...
class Earn extends Component {
	...
	render() {
		const { data, currency, postClaimReward,  addRewardData, deleteRewardData, muiTheme } = this.props
							...
							return (
								<div key={index} className='col-6'>
									<Paper style={style.curr_card} zDepth={3}>
										...
										<div className='container'>
											<img style={style.card_img} src={icon_folder + '/icon1.svg'} alt='earn' />
...
class EarnContainer extends Component {
	...
	render() {
		const { data, loading, postClaimReward,  addRewardData, deleteRewardData, add_result, muiTheme } = this.props
		const user_data = JSON.parse(localStorage.getItem('user'))
...
export default muiThemeable()(connect(mapStateToProps, mapDispatchToProps)(EarnContainer))